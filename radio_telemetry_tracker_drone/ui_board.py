...
import serial
from RCTComms.comms import EVENTS, rctVehiclePacket

from radio_telemetry_tracker_drone.states import (GpsStates, OutputDirStates,
                                                  RctStates, SdrInitStates)
from radio_telemetry_tracker_drone.utils import InstrumentedThread


class UIBoard:
    """UI Board Interface Class
    """
    def __init__(self, port: str = None, baud=115200, test_mode: bool = False) -> None:
        """Stores the current status, most recent GPS and compass info
        ...
        self.__log.info('Initialzed on {} at {} buad, mode={}', port, baud, str(test_mode))
        ...
        self.listener = InstrumentedThread(
            target=self.uib_listener,
            name='UIB Listner',
            daemon=True
        )
        self.recent_loc = None
    ...
    def __init_gps(self):
        if self.test_mode:
            self.sensor_state = GpsStates.RDY
            self.__log.debug('External Sensor State: {}', self.sensor_state)
            return

        self.gps_ready.clear()

        self.sensor_state = GpsStates.GET_TTY
        while self.__port is None:
            try:
                self.__port = serial.Serial(self.__device, baudrate=self.__baud)
            except Exception as exc: # pylint: disable=broad-except
                self.__log.exception('Failed to create serial handle: {}', exc)
                self.sensor_state = GpsStates.FAIL
                time.sleep(1)

        self.sensor_state = GpsStates.GET_MSG
        while True:
            try:
                ...
                self.__log.exception('Failed to read from serial: {}', exc)
                self.sensor_state = GpsStates.FAIL
                continue
            except TimeoutError:
                ...
                self.__log.exception('Bad message: {}', line)
                self.sensor_state = GpsStates.FAIL
                continue
    ...
    def handle_sensor_packet(self, packet):
        ...
        try:
            ...
            self.__log.exception('Sensor Packet callback exception: {}', exc)
    ...
    def storage_state(self, state: Any) -> None:
        ...
        self.__log.warning('Storage state set to {}', OutputDirStates(state).name)
    ...
    def system_state(self, state: Any) -> None:
        ...
        self.__log.info('System state set to {}', RctStates(state).name)
    ...
    def sdr_state(self, state: Any) -> None:
        ...
        self.__log.warning('SDR state set to {}', SdrInitStates(state).name)
    ...
    def switch(self, state: Any) -> None:
        if not isinstance(state, int):
            raise RuntimeError('Illegal type')
        self._switch = state
        self.__log.warning('Switch state set to {}', state)

    def ready(self) -> bool:
        """Evaluates if the system is ready

...
class NetworkMonitor:
    ...
    def register_cb(self, event: Event, cb_: EventCallback) -> None:
        ...
        self.__log.info('Registered {} to {}', cb_, event)
    ...
    def remove_cb(self, event: Event, cb_: EventCallback) -> None:
        ...
        self.__log.info('Removed {} from {}', cb_, event)
        ...
        while not self.__flags[self.Flag.STOP_MONITOR].is_set():
            if self.__profile_name not in self.__current_wifi():
                ...
                try:
                    ...
                    self.__log.warning('Failing to connect to {}', self.__profile_name)
                    error_counter += 1
                    if error_counter >= self.__error_threshold and not error_status:
                        error_status = True
                        self.__log.warning('Failed to connect to {}', self.__profile_name)
                        self.execute_cb(self.Event.ON_NETWORK_LOSS)
            else:
                error_status = False
                self.__flags[self.Flag.NETWORK_SET].set()
                time.sleep(self.__monitor_interval)
                self.__log.debug('Currently connected to {}', self.__profile_name)
...
class _TestMonitorApp:
    ...
    def __on_network_fail(self, event: NetworkMonitor.Event) -> None:
        logging.debug('On Network Fail called with {}', event)
